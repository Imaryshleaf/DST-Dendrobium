local assets =
{
    Asset("ANIM", "anim/fx_dark_shield.zip"),
}

local REPEL_RADIUS = 2
local REPEL_RADIUS_SQ = REPEL_RADIUS * REPEL_RADIUS * REPEL_RADIUS
...
local function StartRepel(inst)
    ...
    local creatures = {}
    for i, v in ipairs(TheSim:FindEntities(x, y, z, REPEL_RADIUS, { "locomotor" }, { "player", "schcaster", "companion", "playerghost", "INLIMBO" })) do
        if v:IsValid() and v.entity:IsVisible() and not (v.components.health ~= nil and v.components.health:IsDead()) then
            ...
            elseif v.components.combat ~= nil then
                v.components.combat:GetAttacked(inst, 10) -- 10 Damage
...
local function MakeShield(name, num, prefabs)
    local function fn()
        ...
        inst.AnimState:SetBuild("fx_dark_shield")
...
local prefs = {}
for i = 1, 4 do
    local name = "fx_dark_shield"..tostring(i)
    table.insert(prefs, name)
    table.insert(ret, MakeShield(name, i))
end
table.insert(ret, MakeShield("fx_dark_shield", nil, prefs))
prefs = nil
