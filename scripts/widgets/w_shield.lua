...
local _oldSetPercent = ShieldBadge.SetPercent
...
function ShieldBadge:OnUpdate(dt)
	...
	if self.owner:HasTag("playerghost") then
		self.anim:Hide() self.shieldarrow:Hide() self.topperanim:Hide() else self.anim:Show() self.shieldarrow:Show() self.topperanim:Show()
	end
end
...
local function ShieldFn(self)
	if self.owner.prefab == "dendrobium" then
		...
		self.shield_widget:SetPosition(-1029, -510, 3)
		self.shield_widget:SetScale(0.75)
