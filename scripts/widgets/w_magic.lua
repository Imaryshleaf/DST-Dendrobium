...
local _oldSetPercent = MagicBadge.SetPercent
...
function MagicBadge:OnUpdate(dt)
	...
	if self.owner:HasTag("playerghost") then
		self.anim:Hide() self.magicarrow:Hide() self.topperanim:Hide() else self.anim:Show() self.magicarrow:Show() self.topperanim:Show()
	end
end
...
local function MagicFn(self)
	if self.owner.prefab == "dendrobium" then
		...
		self.magic_widget:SetPosition(-975, -510, 2)
