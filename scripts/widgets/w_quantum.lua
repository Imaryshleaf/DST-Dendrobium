...
local _oldSetPercent = QuantumBadge.SetPercent
...
function QuantumBadge:OnUpdate(dt)
	...
	if self.owner:HasTag("playerghost") then
		self.anim:Hide() self.quantumarrow:Hide() self.topperanim:Hide() else self.anim:Show() self.quantumarrow:Show() self.topperanim:Show()
	end
end
...
local function QuantumFn(self)
	if self.owner.prefab == "dendrobium" then
		...
		self.quantum_widget:SetPosition(-814, -510, 16)
