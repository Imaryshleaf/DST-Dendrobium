...
-- Shadow: (components check: magic)  -----------------------------------------------------------------------------------
local ShadownFriendsFn = GLOBAL.State {
	name = "ShadownFriendsSG",
	tags = { "doing", "busy" },
	onenter = function(inst)
		inst.components.locomotor:Stop()
		if inst.components.orchidacite and inst.components.orchidacite.min_magic >= (20) then
			inst.AnimState:PlayAnimation("action_uniqueitem_pre")
			inst.AnimState:PushAnimation("book", false)
		else
			inst.sg:GoToState("mindcontrolled")
...
AddStategraphState("wilson", ShadownFriendsFn)
AddStategraphState("wilson_client", ShadownFriendsFn)
