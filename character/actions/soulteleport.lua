...
-- #NOTE: priority 10 is the higher priority

--- Soulhop
local LEAPACT = GLOBAL.Action({ priority = 9, rmb = true, distance = 36, mount_valid = true })
LEAPACT.str = "Leap"
LEAPACT.id = "LEAPACT"
...
AddAction(LEAPACT)
AddStategraphActionHandler("wilson", GLOBAL.ActionHandler(LEAPACT, "leaper_jump_in_pre"))
AddStategraphActionHandler("wilson_client", GLOBAL.ActionHandler(LEAPACT, "leaper_jump_in_pre"))

-- Dodgeslide
local DODGEACT = GLOBAL.Action({ priority = -5 })
DODGEACT.str = "Dodge"
...
local function CheckSoulsAdded(inst)
    ...
    if count > 40 then
        count = count - math.floor(40 / 2) + math.random(0, 2) - 1 -- convert count to drop count
    ...
    elseif count > 30 then
        inst.components.talker:Say("I can't handle that much soul power!")
    end
end
...
local function GetPointSpecialActions(inst, pos, useitem, right)
	...
	if right then
		...
		-- Dodgeslide (remove or add tag "dodger" to character to use)
		if not rider:IsRiding() 
			and inst:HasTag("dodger")
			and GetTime() - inst.last_dodge_time > 2 
			and inst.components.hunger.current > 5 
			and not inst.components.health:IsInvincible() 
			and not inst.components.inventory:Has("soul_orb", 1) then
			return { ACTIONS.DODGEACT }
		end
		
		-- Soulhop (Currently active)
		if not rider:IsRiding()
			and NoHoles(pos)
			and useitem == nil 
			and inst.components.inventory:Has("soul_orb", 1)
			and not inst.components.health:IsInvincible() then
			return { ACTIONS.LEAPACT }
		end
		
		-- Leaverenmant (remove or add tag "remnantsource" to character to use)
		if not rider:IsRiding() 
			and NoHoles(pos)
			and useitem == nil
			and inst:HasTag("remnantsource")
			and GetTime() - inst.last_remnant_time > 5
			and inst.components.sanity.current > 20
			and not inst.components.health:IsInvincible() then
			return { ACTIONS.LEAVEREMNANT }
		end
	end
...
AddPrefabPostInit("dendrobium", function(inst)
	...
	-- Shadowremnant
	inst.last_remnant_time = GetTime()
...
local function ActionPickerPostInit(c)
    ...
    function c:DoGetMouseActions(...)
        ...
        if self.inst:HasTag("soulleaper") then
            return left, right
        end
