local function ContainerPostInit(inst)
	...
	function inst:CanTakeItemInSlot(item, slot)
		if item.components and item.components.classifieditem and not item.components.classifieditem:CanPickUp(inst.opener) and not item.components.classifieditem:IsStorable() then
    		inst.components.talker:Say(item.components.classifieditem:GetComment())
    		inst.opener.components.inventory:DropItem(item)
    		return false
    	end
    	if item.components and item.components.classifieditem and not item.components.classifieditem:CanPickUp(inst.opener) and inst.type == "pack" then
    		inst.components.talker:Say(item.components.classifieditem:GetComment())
    		inst.opener.components.inventory:DropItem(item)
    		return false
    	end
	...
	function inst:TakeActiveItemFromAllOfSlot(slot)
		...
		if item and item.components and item.components.classifieditem and not item.components.classifieditem:CanPickUp(self.opener) then
			inst.opener.components.talker:Say(item.components.classifieditem:GetComment())
    		return 
    	end
